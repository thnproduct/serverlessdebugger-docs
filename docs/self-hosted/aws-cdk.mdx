...
```
After getting the URLs, update your `debug-client.json` by using the `ServerlessDebugger: Edit configuration` command of the ServerlessDebugger's [VSCode Extension](https://github.com/serverlessdebugger/serverlessdebugger-vscode-extension).
...
![Self-Hosted ServerlessDebugger](/img/self-hosted-serverless-debug-broker.png)
